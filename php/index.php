...
<div class="container pt-16">
      ...
      <div class="grid lg:grid-cols-4 sm:grid-cols-2 gap-6">
        <!-- first product start-->
          <!-- product image start -->
          <?php
           $select = "SELECT p.judul_produk, p.gambar_produk1, p.id_produk, p.tersedia, p.harga_produk, p.id_kategori
                      FROM produk p
          ...
          <div class='pt-4 pb-3 px-4'>";
            ...
            <div class='flex items-center'>
              <svg xmlns='http://www.w3.org/2000/svg' fill='' viewBox='0 0 24 24' stroke-width='2' stroke='currentColor' class='w-6 h-6 mr-2 text-yellow-300 fill-yellow-300'>
                            ...
                            <p class='ml-2 mt-[2px] text-white font-semibold'><i>New Arrival</i></p>
            </div>
          </div>
          ";
    ...
    <!-- product start  -->
<div class="container pt-16">
      <h2 class="text-2xl md:text-3xl font-medium text-white uppercase mb-6" data-aos="fade-up" data-aos-duration="500">Best Seller</h2>
      <!-- grid start -->
      <div class="grid lg:grid-cols-4 sm:grid-cols-2 gap-6">
        <!-- first product start-->
          <!-- product image start -->
          <?php
           $select = "SELECT p.judul_produk, p.gambar_produk1, p.tersedia, p.id_produk, p.harga_produk, AVG(l.rating_produk) AS avg_rating, kategori.id_kategori
           FROM produk p
           JOIN kategori ON (p.id_produk = kategori.id_kategori)
           JOIN `order` o ON p.id_produk = o.id_produk
           JOIN laris l ON o.id_order = l.id_order
           GROUP BY p.id_produk
            ORDER BY COUNT(o.id_produk) DESC
            LIMIT 4
            ";
           $hasil = mysqli_query($conn,$select);
           if($hasil->num_rows > 0){
