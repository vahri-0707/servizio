      ...
      //  product button end
        ...
        ?>
          ...
          <?php
             if(isset($_GET['cari'])){
              $value=$_GET['produk'];
              $search = "SELECT DISTINCT kategori.id_kategori, kategori.kategori, produk.id_produk, produk.judul_produk, produk.gambar_produk1, produk.harga_produk, produk.tersedia, AVG(rating_produk) AS avg_rating
              FROM kategori 
              JOIN produk ON kategori.id_kategori = produk.id_kategori
              LEFT JOIN `order` ON produk.id_produk = `order`.id_produk
              LEFT JOIN laris ON `order`.id_order = laris.id_order
